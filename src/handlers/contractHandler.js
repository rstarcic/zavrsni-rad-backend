...
async function fetchDataForContract(jobId, serviceProviderId, clientId) {
    try {
        const serviceProviderData = await ServiceProvider.findByPk(serviceProviderId, { attributes: ['id', 'firstName', 'lastName', 'address', 'city'] });
        ...
        if (jobData.duration && jobData.hourlyRate && jobData.workingHours) {
            amount = calculateTotalAmount(jobData.duration, jobData.hourlyRate, jobData.workingHours)
        }
...
function calculateTotalAmount(duration, hourlyRate, workingHours) {
...
async function _savePDFContractToDatabase(pdfData, jobAdId) {
...
async function generateContract(res, signature, contractData) {
    ...
    doc.on('end', async () => {
        ...
        const jobContract = await _savePDFContractToDatabase(pdfData, jobData.id);
    ...
    const court = `${serviceProviderData.city}`;
    const bank = "OTP bank";
    const accountNumber = "HR122345446464";
    ...
    doc.text(court, { align: 'center' }).moveUp();
    ...
    doc.moveDown(3);
    if (signature) {
        doc.image(signature, { fit: [200, 60], align: 'left' });
    }
    if (signature) {
...
export {
    fetchDataForContract, generateContract
}
